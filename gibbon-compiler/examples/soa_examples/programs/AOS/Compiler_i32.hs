...
data IR
  = Instr Int32   -- opcode (see encoding below)
          Int32   -- flags
          Int32   -- src1
          Int32   -- src2
          Int32   -- dst
...
buildIR_validPhi_go n pendingPhi =
  ...
  then
    ...
    Instr (toInt32 op) (toInt32 flags) (toInt32 (n-1)) (toInt32 (n-2))
          (toInt32 n) (toInt32 lat) (toInt32 thr)
          (buildIR_validPhi_go (n - 1) 0)
...
goHasCycle ir curBlock =
  case ir of
    Instr op _ tgt _ _ _ _ rest ->
      let isBackedge = (op == 4) && (toInt64 tgt < curBlock)
...
latencyModelPass ir =
  case ir of
    Instr _ _ _ _ _ lat _ rest ->
      toInt64 lat + latencyModelPass rest
    BlockEnd rest ->
...
throughputModelPass ir =
  case ir of
    Instr _ _ _ _ _ _ thr rest ->
      toInt64 thr + throughputModelPass rest
    BlockEnd rest ->
...
targetRetunePass ir k =
  case ir of
    Instr op fl s1 s2 dst lat thr rest ->
      Instr op fl s1 s2 dst (toInt32 (toInt64 lat * k)) thr

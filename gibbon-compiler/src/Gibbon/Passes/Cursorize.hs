  ...
  mainExp' <- case mainExp of
    ...
    Just (e, ty) -> do
      if hasPacked (unTy2 ty)
        then
          do 
            (e', _, _, _) <- cursorizePackedExp M.empty M.empty userRequestedMutableCursors False M.empty M.empty ddefs fundefs M.empty M.empty M.empty e
            Just . (,stripTyLocs (unTy2 ty))
              <$> fromDi
              <$> return e' 
        else
          do 
          (e', _, _, _) <- cursorizeExp M.empty M.empty userRequestedMutableCursors False M.empty M.empty ddefs fundefs M.empty M.empty M.empty e
          Just . (,stripTyLocs (unTy2 ty))
            <$> return e'
...
handleIndexingSoACursors tenv mbvarlarg m1 m2 forceMutable (arrLoc, arrName) (start, end) locvar var_env = do
                                           ...
                                           case arrLoc of
                                                ...
                                                SoA{} -> do
                                                         ...
                                                         let make_cur_arr_let = [(arrName, [], getCursorizeTyFromLocVar Nothing forceMutable arrLoc, Ext $ MakeCursorArray (length vars) vars)]
...
handleIndexingSoARegCursors forceMutable (arrLoc, arrName) (start, end) locvar var_env = do
                                           ...
                                           case arrLoc of 
                                                ...
                                                SoARv{} -> do
                                                         ...
                                                         let make_cur_arr_let = [(arrName, [], getCursorizeTyFromRegVar''' Nothing forceMutable arrLoc, Ext $ MakeCursorArray (length vars) vars)]
...
cursorizeFunDef ddefs fundefs FunDef {funName, funTy, funArgs, funBody, funMeta} = do
  ...
  -- && isFunTailRec
  let useMutableCursors = userRequestedMutableCursors && isFunRec
...
cursorizeExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
    ...
    AppE {} -> cursorizeAppE m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex
      ...
      case M.lookup v tenv of
        ...
        Just ty ->
          if isPackedTy (unTy2 ty)
            ...
            else pure $ (LitE $ fromJust $ sizeOfTy (unTy2 ty), freeVarToVarEnv, m1, m2)
    PrimAppE pr args -> do 
    ...
    LetE (v, _locs, _ty, (PrimAppE (ReadPackedFile path tyc reg ty2) [])) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeReadPackedFile m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv True v path tyc reg ty2 bod
    ...
    LetE (_v, _locs, _ty, (MkProdE _ls)) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_prod, freeVarToVarEnv'', m1', m2') <- cursorizeProd m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
    ...
    LetE (_v, _locs, ty, ProjE {}) _bod | isPackedTy (unTy2 ty) -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeProj m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return $ (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    LetE (_v, _locs, _ty, SpawnE {}) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeSpawn m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    LetE (_v, _locs, _ty, SyncE) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeSync m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return $ (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    ...
    LetE bnd@(_, _locs, _, _) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeLet m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv bnd bod
    ...
    CaseE scrt brs -> do
      ...
      case ty_of_scrut of
        ...
        MutCursorTy -> do
                       ...
                       let (output_mut_loc_scrut, mupdated) = findMutableLocationPointingToVar v m1
                       (m1', m2') <- case output_mut_loc_scrut of 
                                            Nothing -> error "Did not expect mutable cursor!\n"
                                            Just outloc -> do let m1i = updateMutableLocPtsToEnv outloc mupdated (deref_val, Just outloc, Nothing, S.empty) False
        ...
        CursorArrayTy {} -> do
          ...
          let (mut_loc_pointing_to_v, m1updated) = findMutableLocationPointingToVar v m1
          (dcon_var', dcon_let, m1', m2', freeVarToVarEnv'') <- case mut_loc_pointing_to_v of 
                                                            ...
                                                            Just ml -> do
                                                                        ...
                                                                        let m1i = updateMutableLocPtsToEnv dcon_loc m1updated (dcon_var_deref, Just dcon_loc, Nothing, S.empty) True
    ...
    WithArenaE v e -> do
      (e', env, m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv (M.insert v (MkTy2 ArenaTy) tenv) senv e
    ...
    Ext ext ->
      case ext of
          ...
          let (bnds, tenv', m1mextended) = dbgTrace (minChatLvl) "Print envs after cursorizeLocExp: " dbgTrace (minChatLvl) (sdoc (m1', m2')) dbgTrace (minChatLvl) "End print envs after cursorizeLocExp.\n" case M.lookup (fromLocVarToFreeVarsTy loc) denv of
                ...
                Just vs ->
                  let extended = M.fromList [(v, MkTy2 CursorTy) | (v, _, CursorTy, _) <- vs]
                      mextended = foldr (\((v, _, _, _)) mfld -> let (mutloc, m1updated) = findMutableLocationPointingToVar locs_variable mfld
                                                                          in case mutloc of 
                                                                              Nothing -> m1updated
                                                                              Just ml -> updateMutableLocPtsToEnv ml m1updated (v, Just ml, Nothing, S.empty) True        
                                        ) m1' vs 
        ...
        LetRegionE reg sz endmut _ bod -> do
          ...
          (bod, freeVarToVarEnv'''', m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv''' lenv ddfs fundefs denv tenv'' senv bod
          ...
          case rhs_either of
            Right (rhs', bnds', tenv'', senv') -> do
              ...
              case rhs of
                ...
                _ -> case ty_of_loc of
                  MutCursorTy -> do
                       (bod', env, m1'', m2'') <- cursorizeExp m1' m2' useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv (M.insert locs_var (ty2_of_loc) tenv''') senv' bod
            ...
            Left denv' -> do
                          (bod', env, m1'', m2'') <- cursorizeExp m1' m2' useMutableCursorsCall insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv' tenv' senv bod
  ...
  where
    go insidetimeit gm1 gm2 fenv = cursorizeExp gm1 gm2 useMutableCursorsCall insidetimeit fenv lenv ddfs fundefs denv tenv senv
...
cursorizePackedExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
    ...
    AppE {} -> do
               (ex', freeVarToVarEnv', m1', m2') <- cursorizeAppE m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex
    ...
    LetE (v, _locs, _ty, (PrimAppE (ReadPackedFile path tyc reg ty2) [])) bod -> do
      (bod', freeVarToVarEnv', m1', m2') <- cursorizeReadPackedFile m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv True v path tyc reg ty2 bod
      return (Di bod', freeVarToVarEnv', m1', m2') 
    LetE (v, _locs, _ty, (PrimAppE (DictLookupP (MkTy2 (PackedTy _ ploc))) vs)) bod ->
      do
        vs' <- forM vs $ \w -> cursorizeExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv w
    ...
    LetE (_v, _locs, _ty, (MkProdE _ls)) _bod -> do
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeProd m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    LetE (_v, _locs, ty, ProjE {}) _bod
      | isPackedTy (unTy2 ty) -> do
          (ex', freeVarToVarEnv', m1', m2') <- cursorizeProj m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
          return (dl ex', freeVarToVarEnv', m1', m2') 
    MkProdE ls -> do
        ...
        forM (zip tys ls) $ \(ty, e) -> do
          case ty of
            _ | isPackedTy (unTy2 ty) -> do 
                                        (e', freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv e
                                        return (fromDi e', freeVarToVarEnv', m1', m2') 
            _ -> cursorizeExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv e
      let es = map fst4 res
      let envs = map snd4 res
      let m1s' = map thd4 res
      let m2s' = map fth4 res
    ...
    LetE (_v, _locs, _ty, SpawnE {}) _bod -> do 
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeSpawn m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    LetE (_v, _locs, _ty, SyncE) _bod -> do
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeSync m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    ...
    LetE bnd@(_, _locs, _, _) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (bod', freeVarToVarEnv'', m1', m2') <- cursorizeLet m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv' lenv True ddfs fundefs denv tenv senv bnd bod    
    ...
    IfE a b c -> do
      ...
      (a', env3, m1''', m2''') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv a
      let m1'''' = M.unions [m1', m1'', m1''']
      let m2'''' = M.unions [m2', m2'', m2''']
      return (Di $ IfE a' b' c', M.unions [env1, env2, env3], m1'''', m2'''')
    ...
    CaseE scrt brs -> do
      ...
      case ty_of_scrut of
        ...
        MutCursorTy -> do
                       ...
                       let additional_deref_let = mkLets [(deref_val, [], CursorTy, Ext $ DerefMutCursor v)]
                       let tenv' = M.insert deref_val (MkTy2 CursorTy) tenv
                       let (output_mut_loc_scrut, m1updated) = dbgTrace (minChatLvl) "Print in MutCursor Case: " dbgTrace (minChatLvl) (sdoc (ty_of_scrut, v, alive_buffers)) dbgTrace (minChatLvl) "End print in MutCursor Case.\n" findMutableLocationPointingToVar v m1
                       (m1', m2') <- case output_mut_loc_scrut of 
                                            Nothing -> error "Did not expect mutable cursor!\n"
                                            Just outloc -> do let m1i = updateMutableLocPtsToEnv outloc m1updated (deref_val, Just outloc, Nothing, S.empty) False 
                                                              let m2i = M.insert outloc (deref_val, Just outloc, Nothing, S.empty) m2
                                                              return (m1i, m2i)
        ...
        then do
          ...
          let go2 ::  MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Var -> [(Exp2, Ty2)] -> PassM (Exp3,  MutableLocPtsToEnv, MutableLocOldValueEnv)
              ...
              go2 mg1 mg2 marker_added d ((rnd, (MkTy2 ty)) : rst) = do
                ...
                case ty of
                  ...
                  _ | isScalarTy ty -> do
                    (rnd', freeVarToVarEnv', m1', m2') <- cursorizeExp mg1 mg2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv rnd
                    let (mut_loc_pts, m1updated) = dbgTrace (minChatLvl) "Print in DataConE: " dbgTrace (minChatLvl) (sdoc (additional_bnds, m1, m1')) dbgTrace (minChatLvl) "End in DataConE Case.\n" findMutableLocationPointingToVar d m1'
                    (additional_bnds, m1'') <- case mut_loc_pts of 
                                                      Nothing -> return ([], m1updated)
                                                      Just ml -> do
                                                                  ...
                                                                  let m1updated' = updateMutableLocPtsToEnv ml m1updated (d', Just ml, Nothing, S.empty) False
                                                                  return (add_bnds, m1updated') 
                  ...
                  -- shortcut pointer
                  CursorTy -> do
                    (rnd', freeVarToVarEnv', m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv ddfs fundefs denv tenv senv rnd
          ...
          let sloc_loc = toLocVar slocarg
              ...
              (sloc_to_write, sloc, sloc_dcon, present, freeVarToVarEnv') = if isMutModality $ fromJust modality_sloc
                                                                           then
                                                                            let (sl_loc, sl_val_old) = case M.lookup sloc_loc m2 of 
                                                                                ...
                                                                                dcon_var = case M.lookup dcon_loc m2 of
              ...
              go2 marker_added mdc1 mdc2 fvarenv aft_dloc from_rec_end aft_flocs [] = do
                ...
                if not (marker_added)
                  then do
                    ...
                    let makeCurArr = Ext $ MakeCursorArray (1 + length (after_flocs_to_vars)) ([aft_dloc] ++ after_flocs_to_vars)
              ...
              go2 marker_added mdc1 mdc2 fvarenv aft_dloc from_rec_end aft_flocs (((dcon, index), floc, (rnd, (MkTy2 ty))) : rst) = do
                ...
                case ty of
                  ...
                  _ | isScalarTy ty -> do
                    (rnd', fvarenv', m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeit fvarenv lenv ddfs fundefs denv tenv senv rnd
                    ...
                    let (mut_loc_pointing_at_scalar, mdc1updated) = findMutableLocationPointingToVar floc_var' mdc1
                    mdc1' <- case mut_loc_pointing_at_scalar of 
                                              Nothing -> return mdc1updated
                                              Just ml -> do
                                                          let mdc1i = updateMutableLocPtsToEnv ml mdc1updated (write_scalars_at, Just ml, Nothing, S.singleton floc_var') True
                                                          return mdc1i 
                    ...
                    let (mut_loc_pointing_at_write_scalar, mdc1prprupdated) = findMutableLocationPointingToVar write_scalars_at mdc1''
                    (bump_bnds, mdc1''') <- case mut_loc_pointing_at_write_scalar of 
                                               Nothing -> return ([], mdc1prprupdated) 
                                               Just ml -> do 
                                                           let mdc1i = updateMutableLocPtsToEnv ml mdc1prprupdated (d', Just ml, Nothing, S.empty) False
                                                           void_var <- gensym "void"
          ...
          (additional_bnds', freeVarToVarEnv'', _) <-
            foldlM
              ...
              field_locs
          let (mut_loc_sloc_dcon, m1updated) = findMutableLocationPointingToVar sloc_dcon m1
          (mut_loc_ad_bnds, m1') <- dbgTrace (minChatLvl) "Print in SoA Dcon case: " dbgTrace (minChatLvl) (sdoc (m1, sloc_dcon)) dbgTrace (minChatLvl) "End in print dcon case SoA!!\n" case mut_loc_sloc_dcon of 
                                              ...
                                              Just ml -> do 
                                                          ...
                                                          let mi1 = updateMutableLocPtsToEnv ml m1updated (after_tag, Just ml, Nothing, S.empty) False
                                                          return (bnd, mi1)
      ...
      case ext of
        ...
        LetLocE locarg rhs bod -> do
          ...
          let (bnds, tenv', m1extended) = dbgTrace (minChatLvl) "Print envs after cursorizeLocExp: " dbgTrace (minChatLvl) (sdoc (m1', m2')) dbgTrace (minChatLvl) "End print envs after cursorizeLocExp.\n" case M.lookup (fromLocVarToFreeVarsTy loc) denv of
                ...
                Just vs ->
                  let extended = M.fromList [(v, MkTy2 CursorTy) | (v, _, CursorTy, _) <- vs]
                      mextended = foldr (\((v, _, _, _)) mfld -> let (mutloc, mfldupdated) = findMutableLocationPointingToVar locs_var mfld
                                                                          in case mutloc of 
                                                                              Nothing -> mfldupdated
                                                                              Just ml -> updateMutableLocPtsToEnv ml mfldupdated (v, Just ml, Nothing, S.empty) True        
                                        ) m1' vs                      
                   in (vs, M.union extended tenv, mextended)
          case rhs_either of
            Right (rhs', bnds', bnds_after, tenv'', senv') -> do
            ...
            Left denv' -> do
              (bod', freeVarToVarEnv'', m1'', m2'') <- cursorizePackedExp m1extended m2' useMutableCursorsCall insideTimeit freeVarToVarEnv' lenv ddfs fundefs denv' tenv' senv bod
        ...
        LetRegE loca rhs bod -> do
          ...
          case rhs_either of
            ...
            Left denv' -> do 
                (bod', freeVarToVarEnv'', m1'', m2'') <- cursorizePackedExp m1' m2' useMutableCursorsCall insideTimeit freeVarToVarEnv' lenv ddfs fundefs denv' tenv' senv bod
                return (onDi (mkLets bnds) bod', freeVarToVarEnv'', m1'', m2'')
        -- case reg_var of
        -- SingleR v -> cursorizePackedExp freeVarToVarEnv ddfs fundefs denv tenv senv bod
        -- SoARv dv _ -> cursorizePackedExp freeVarToVarEnv ddfs fundefs denv tenv senv bod
  ...
  where
    go intimit menv1 menv2 env = cursorizePackedExp menv1 menv2 useMutableCursorsCall intimit env lenv ddfs fundefs denv
    dl = Di
...
mutLocNeedsBump freeVarToVarEnv ptsEnv oldEnv lv v offset = do 
                                       let mutBump = L.foldr (\(kl, lst) mred -> 
                                                                        foldr (\ent mred' -> 
                                                                                  case ent of 
                                                                                       CurrentScope (vpts, lpts, _endreg, _aliases) ->  
                                                                                                                  case v of 
                                                                                                                      Just vv -> if (vv == vpts)
                                                                                                                                 then Just kl
                                                                                                                                 else mred'
                                                                                       FutureScope (vpts, lpts, _endreg, _aliases) ->  
                                                                                                                  case v of 
                                                                                                                      Just vv -> if (vv == vpts)
                                                                                                                                 then Just kl
                                                                                                                                 else mred'
...
cursorizeReadPackedFile ::
  ...
  Exp2 ->
  PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeReadPackedFile m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv isPackedContext v path tyc reg ty2 bod = do
  case reg of
  ...
  where
    go intime m1g m2g t e =
      if isPackedContext
        then 
          do 
           (e', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv e
           return (fromDi e', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1g m2g useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv e
...
cursorizeLocExp mLocPtsToEnv mLocOldValEnv useMutableCursorsCall freeVarToVarEnv denv tenv senv lvararg locExp =
  case locExp of
    AfterConstantLE i loc -> do
      ...
      let (mut_loc_var, mLocPtsToEnvUpdated) = findMutableLocationPointingToVar loc_var_aftc mLocPtsToEnv
      ...
      (locs_var, use_this_loc, additional_bnds, bnds_after, mLocPtsToEnv', mLocOldValEnv') <- case ((isMutModality $ fromJust $ getModality loc) || (Mb.isJust mut_loc_var && (isInputModality (getModality loc)))) of 
                                                                    ...
                                                                    True -> if (M.member (toLocVar loc) mLocOldValEnv)
                                                                            then do 
                                                                              ...
                                                                              needs_bump <- mutLocNeedsBump freeVarToVarEnv mLocPtsToEnvUpdated mLocOldValEnv loc_pts (Just vl) (L3.LitE i)
                                                                              case needs_bump of
                                                                                Nothing -> if (M.member (toLocVar loc) mLocPtsToEnvUpdated)
                                                                                            -- The locations is already in the PtsToEnv 
                                                                                            -- therefore, we gets its value from the oldenv
                                                                                            then let old_val = dbgTrace (minChatLvl) "Print tailrecimplt in cursorizeLocExp:" dbgTrace (minChatLvl) (sdoc (mLocPtsToEnvUpdated, mLocOldValEnv, locExp)) dbgTrace (minChatLvl) "End printing tailrecimplt in cursorizeLocExp Nothing case1.\n" M.lookup (toLocVar loc) mLocOldValEnv
                                                                                              in case old_val of 
                                                                                                        Nothing -> error "Expected to have a value for the mutable location!"
                                                                                                        -- VS: No need to bump the mutable location in case we use the old location.
                                                                                                        Just (vl, Just l, _, _) -> do 
                                                                                                                  -- let l_name = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy l) 
                                                                                                                  pure (vl, l, [], [], mLocPtsToEnvUpdated, mLocOldValEnv)
                                                                                                        -- If the loc is not available, we just use the mut loc
                                                                                                        -- TODO. this may not be required / special handling?
                                                                                                        Just (vl, Nothing, _, _) -> do
                                                                                                                  pure (vl, (toLocVar loc), [], [], mLocPtsToEnvUpdated, mLocOldValEnv) 
                                                                                            ...
                                                                                            -- value of the OutputMutable location.
                                                                                              ...
                                                                                              let bumpMutLoc = (bump_loc_var, [], ProdTy [], Ext $ BumpCursorMutable loc_name (LitE i))
                                                                                              -- We need to make the mutable loc point to the dereferenced value 
                                                                                              let mLocPtsToEnv'' = updateMutableLocPtsToEnv (toLocVar loc) mLocPtsToEnvUpdated (lvar_name, Just lvar, Just $ toEndRegVar lvararg, S.empty) False
                                                                                              -- if there is no mapping of the mutable loc to its old value, we need to update it.
                                                                                              let mLocOldValEnv'' = if (M.member (toLocVar loc) mLocOldValEnv)
                                                                                                                  then mLocOldValEnv
                                                                            ...
                                                                            then
                                                                              ...
                                                                              -- value of the OutputMutable location.
                                                                                ...
                                                                                -- We need to make the mutable loc point to the dereferenced value 
                                                                                let mLocPtsToEnv'' = updateMutableLocPtsToEnv mut_loc mLocPtsToEnvUpdated (lvar_name, Just mut_loc, Nothing, S.empty) True
                                                                                -- if there is no mapping of the mutable loc to its old value, we need to update it.
                                                                    ...
                                                                    _ -> do
                                                                          ...
                                                                          (new_key_vals, after_bnds) <- foldlM (\(kvals, bnds) (key, lst) -> 
                                                                                                                      foldlM (\(kvals', bnds') ent ->
                                                                                                                                 case ent of 
                                                                                                                                   CurrentScope (vval, lval, endreg, aliases) -> 
                                                                                                                                            case lval of 
                                                                                                                                                  Nothing -> if vval == loc_name
                                                                                                                                                             then
                                                                                                                                                              ...
                                                                                                                                                              return (kvals' ++ [(key, (lst ++ [CurrentScope (lvar_name, Just lvar, Just $ toEndRegVar lvararg, aliases)]))], bnds' ++ [aft_bnd])
                                                                                                                                                             else return $ (kvals' ++ [(key, (lst ++ [CurrentScope (vval, lval, endreg, aliases)]))], bnds')
                                                                                                                                                  Just l -> if l == (toLocVar loc)
                                                                                                                                                            then
                                                                                                                                                              ...
                                                                                                                                                              return $ (kvals' ++ [(key, (lst ++ [CurrentScope (vval, lval, endreg, aliases)]))], bnds')
                                                                                                                                   FutureScope (vval, lval, endreg, aliases) -> 
                                                                                                                                            case lval of 
                                                                                                                                                  Nothing -> if vval == loc_name
                                                                                                                                                             then
                                                                                                                                                              do
                                                                                                                                                              ...
                                                                                                                                                              bump_loc_var <- gensym "void"    
                                                                                                                                                              let aft_bnd = (bump_loc_var, [], ProdTy [], Ext $ BumpCursorMutable key_name (LitE i)) 
                                                                                                                                                              return (kvals' ++ [(key, (lst ++ [FutureScope (lvar_name, Just lvar, Just $ toEndRegVar lvararg, aliases)]))], bnds' ++ [aft_bnd])
                                                                                                                                                             else return $ (kvals' ++ [(key, (lst ++ [FutureScope (vval, lval, endreg, aliases)]))], bnds')
                                                                                                                                                  Just l -> if l == (toLocVar loc)
                                                                                                                                                            then
                                                                                                                                                              do
                                                                                                                                                              let key_name = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy key)
                                                                                                                                                              ...
                                                                                                                                                              return $ (kvals' ++ [(key, (lst ++ [FutureScope (vval, lval, endreg, aliases)]))], bnds')
                                                                                                                          ) (kvals, bnds) lst
                                                                                                                                ) ([], []) (M.toList mLocPtsToEnvUpdated)
                                                                          let mLocPtsToEnv'' = dbgTrace (minChatLvl) "Print tailrecimplt in cursorizeLocExp:" dbgTrace (minChatLvl) (sdoc (new_key_vals, after_bnds, locExp, mLocPtsToEnvUpdated, mLocOldValEnv)) dbgTrace (minChatLvl) "End printing tailrecimplt in cursorizeLocExp False.\n" M.fromList new_key_vals
                                                                          let loc_name = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy (toLocVar loc))
    ...
    AfterVariableLE v locarg was_stolen -> do
      ...
      let vty = case M.lookup v tenv of
          ...
          (mut_loc_pointing_to_loc, mLocPtsToEnvUpdated) = findMutableLocationPointingToVar (toEndV v) mLocPtsToEnv
          mLocPtsToEnv' = case mut_loc_pointing_to_loc of 
                                      Nothing -> mLocPtsToEnvUpdated
                                      Just l -> let m1 = updateMutableLocPtsToEnv l mLocPtsToEnvUpdated (lvar_name, Just lvar, Nothing, S.empty) False
                                                 in m1
    ...
    FromEndLE locarg -> do
      let loc = toLocVar locarg
          ...
          lvar_name = case (M.lookup (fromLocVarToFreeVarsTy lvar) freeVarToVarEnv) of
            Just v -> v
            Nothing -> error $ "cursorizeRegExp: GetDataConRegSoA: unexpected location variable: " ++ "(" ++ show locExp ++ "," ++ (show (lvar)) ++ ")" ++ show freeVarToVarEnv
          (mut_loc_pointing_to_loc, mLocPtsToEnvUpdated)  = findMutableLocationPointingToVar locs_var mLocPtsToEnv
      mLocPtsToEnv' <- case mut_loc_pointing_to_loc of 
                                    Nothing -> return mLocPtsToEnvUpdated
                                    Just mloc -> do 
                                                  let m1' = updateMutableLocPtsToEnv mloc mLocPtsToEnvUpdated (lvar_name, Just mloc, Nothing, S.singleton locs_var) True
                                                  return m1'
      if isBound locs_var tenv
    ...
    GetDataConLocSoA loc -> do
      ...
      (mLocPtsToEnv', mLocOldValEnv', rhs, additional_bnds) <- case (isMutModality' modality) of 
                                                True -> do 
                                                                      let rhs_tmp = Ext $ AddrOfCursor $ Ext $ IndexCursorArray loc_var 0 
                                                ...
                                                _ -> do
                                                     let rhs_tmp = Ext $ IndexCursorArray loc_var 0  
      ...
      if isBound loc_var tenv
            ...
            else pure $ (Left $ M.insertWith (++) (fromLocVarToFreeVarsTy loc_from_logarg) (additional_bnds ++ [(lvar_name, [], CursorTy, rhs)]) denv,  mLocPtsToEnv', mLocOldValEnv')
    GetFieldLocSoA i loc -> do
      ...
      let loc_from_locarg = toLocVar loc
          ...
          elem_idx = case (L.elemIndex field_loc_elem field_locs) of
            Just idx -> idx
            Nothing -> error "cursorizeLocExp: GetFieldLocSoA: field location not found!"
          lvar_name = dbgTrace (minChatLvl) "Print in GetFieldLocSoA: " dbgTrace (minChatLvl) (sdoc (mLocPtsToEnv, loc, modality, lvar)) dbgTrace (minChatLvl) "End in GetFieldLocSoA.\n" case (M.lookup (fromLocVarToFreeVarsTy lvar) freeVarToVarEnv) of
            Just v -> v
            Nothing -> error $ "cursorizeRegExp: GetDataConRegSoA: unexpected location variable: " ++ "(" ++ show locExp ++ "," ++ (show (lvar)) ++ ")" ++ show freeVarToVarEnv
      -- () <- case modality of 
      ...
      (mLocPtsToEnv', mLocOldValEnv', rhs, additional_lets, lets_before) <- case (isMutModality' modality && not (M.member lvar mLocPtsToEnv)) of 
                                                                      True -> do 
                                        ...
                                        Single{} -> case modality_floc of 
                                                            ...
                                                            Just InputMutable -> case M.lookup locfromlocarg mLocPtsToEnv of 
                                                                                                ...
                                                                                                Just lst -> case lst of
                                                                                                                  ...
                                                                                                                  -- TODO: Check this
                                                                                                                  CurrentScope (var, _,_, _):xs -> case M.lookup var tenv of 
                                                                                                                                        Nothing -> pure $ [(var, [])]
                                                                                                                                        Just ty -> case (unTy2 ty) of 
                                                                                                                                                      MutCursorTy -> do 
                                                                                                                                                      ...
                                                                                                                                                      CursorTy -> pure $ [(var, [])]
                                                                                                                  _ -> error "Expected to have a Current scope!!"
       ...
       then dbgTrace (minChatLvl) "Print in findMutSoALocPtsToSoALoc: " dbgTrace (minChatLvl) (sdoc (dconloc, dconlocmut, fldlocs, fldlocs')) dbgTrace (minChatLvl) "End in findMutSoALocPtsToSoALoc!\n" env -- error $ "Did not expected null!" ++ show (dconloc, dconlocmut, fldlocs, fldlocs')
        ...
        let constructed_soa_loc = SoA (unwrapLocVar $ fromJust dconlocmut) (map (\(k, l) -> (k, fromJust l)) fldlocs')
         ...
         in case mutsoa of 
                    ...
                    Just ls -> case ls of
                             CurrentScope (_var, _l, r, aliases):xs -> dbgTrace (minChatLvl) "Print in findMutSoALocPtsToSoALoc: " dbgTrace (minChatLvl) (sdoc (dconloc, dconlocmut, fldlocs, fldlocs')) dbgTrace (minChatLvl) "End Not null in findMutSoALocPtsToSoALoc!\n" updateMutableLocPtsToEnv constructed_soa_loc env (locsvar, Just locvar, r, S.insert _var aliases) False
                             -- Vidush: Debug why we are having a lst starting with a Future scope!!!
                             FutureScope (_var, _l, r, aliases):xs -> dbgTrace (minChatLvl) "Print in findMutSoALocPtsToSoALoc: " dbgTrace (minChatLvl) (sdoc (dconloc, dconlocmut, fldlocs, fldlocs')) dbgTrace (minChatLvl) "End Not null in findMutSoALocPtsToSoALoc!\n" updateMutableLocPtsToEnv constructed_soa_loc env (locsvar, Just locvar, r, S.insert _var aliases) False
                             _ -> error $ "Expected to have variable keys in env!\n" ++ show ls
...
cursorizeAppE :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeAppE m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
    AppE f _cty locs args -> do
      ...
      (freeVarToVarEnv', newInsts) <-
        foldrM
                          ...
                          (fieldReg_vars, bnds) <-
                                foldlM
                                  (\(vs, bds) (key, field_reg) -> do 
                                                                    ...
                                                                    case field_reg of 
                                                                          SingleR{} -> do 
                                                                                       let (idx, _, _) = getIndexPositionOfSoARegVar useMutableCursorsCall Nothing fieldRegions field_reg
                                  ...
                                  fieldRegions
                          name <- gensym "cursor_reg_ptr"
                          let instrs = dcon_insts ++ (map (\i -> LetE i) bnds) ++ [LetE (name, [], getCursorizeTyFromRegVar Nothing useMutableCursorsCall r, Ext $ MakeCursorArray (1 + length fieldReg_vars) ([dconReg_var] ++ fieldReg_vars))]
          ...
          (freeVarToVarEnv, [])
          locs
      let isFunctionRec = case funRec fmeta of 
                                      TailRec -> True
                                      Rec -> True 
                                      _ -> False
      args' <-
          ...
          ( \(t, a) -> case a of 
                          VarE av -> 
                                  if hasPacked (unTy2 t)
                                    then 
                                      do 
                                        let (mut_loc, m1updated) = findMutableLocationPointingToVar av m1
                                        case mut_loc of 
                                              Nothing -> do  
                                                         (a', _, _, _) <- cursorizePackedExp m1updated m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                                              ...
                                              Just l ->  do 
                                                         ...
                                                         (a', _, _, _) <- cursorizePackedExp m1updated m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv (VarE var)
                                                         dbgTrace (minChatLvl) "Print args in AppE: " dbgTrace (minChatLvl) (sdoc (a, a', args)) dbgTrace (minChatLvl) "End printing in AppE 2.\n" fromDi <$> return a'
                                    else do 
                                        let (mut_loc, m1updated) = findMutableLocationPointingToVar av m1
                                        case mut_loc of 
                                              Nothing -> do  
                                                         (a', _, _, _) <- cursorizeExp m1updated m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                                                         dbgTrace (minChatLvl) "Print args in AppE: " dbgTrace (minChatLvl) (sdoc (a, a', args, (map unTy2 argTys))) dbgTrace (minChatLvl) "End printing in AppE 1.\n" return a'
                                              Just l ->  do 
                                                         let var = getVarNameFromFreeVar freeVarToVarEnv' (fromLocVarToFreeVarsTy l) 
                                                         (a', _, _, _) <- cursorizeExp m1updated m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv (VarE var)
                          ...
                          _ -> if hasPacked (unTy2 t)
                                    then 
                                      do 
                                        (a', _, _, _) <- cursorizePackedExp m1 m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                                        fromDi <$> return a'
                                    else do 
                                        (a', _, _, _) <- cursorizeExp m1 m2 (useMutableCursorsCall && isFunctionRec) insideTimeIt freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                                        return a'
          )
          (zip in_tys args)
      starts <- mapM (\(ty, arg) -> giveStarts tenv freeVarToVarEnv useMutableCursorsCall insideTimeIt isFunctionRec m1 m2 ty arg) (zip (map unTy2 argTys) args')
      -- let loc_var = toLocVar loc
                    ...
                    (\(bnds, args) loc -> do
                        ...
                        case mutl of 
                                  ...
                                  Nothing -> do 
                                             case (getModality loc) of 
                                                      Nothing -> do
                                                                 ...
                                                                 let (mut_l, _m1updated) = findMutableLocationPointingToVar lName m1
                                                                 case mut_l of 
                                                      ...
                                                      Just Input -> do
                                                                    case useMutableCursorsCall of 
                                                                             True -> do  
                                                                                      let varName = getVarNameFromFreeVar freeVarToVarEnv' (fromLocArgToFreeVarsTy loc)
                                                                                          (mut_l_pointing_to_cur, m1updated) = dbgTrace (minChatLvl) "Print in AppE Just Input: " dbgTrace (minChatLvl) (sdoc (varName, loc)) dbgTrace (minChatLvl) "End print in AppE Just Input arg.\n" findMutableLocationPointingToVar varName m1
                                                                                      case mut_l_pointing_to_cur of 
                                                                                                             -- Vidush: Again just getting the first variable
                                                                                                  Just ml -> let CurrentScope (rvar,_,_,_):xs = fromJust $ M.lookup ml m1updated
                                                      ...
                                                      Just _ -> do
                                                                 let loc_name = dbgTrace (minChatLvl) "Print in Just _ case: " dbgTrace (minChatLvl) (sdoc (mutl, loc)) dbgTrace (minChatLvl) "End in Just _ case." (getVarNameFromFreeVar freeVarToVarEnv' (fromLocArgToFreeVarsTy loc))
                                                                     (mut_l_pointing_to_cur, _m1updated) = findMutableLocationPointingToVar loc_name m1 
                                  ...
                                  Just mutl' -> case (isRegionLocArg loc) of 
                                                          ...
                                                          True -> let CurrentScope (rvar,_,_,_):xs = fromJust $ M.lookup mutl' m1 
...
-}
cursorizeProj :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeProj m1 m2 useMutableCursorsCall insideTimeit freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex =
  case ex of
  ...
  where
    go intime m1g m2g t x =
      if isPackedContext
        then do 
           (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv x
           return (fromDi x', freeVarToVarEnv', m1g', m2g')
        else cursorizeExp m1g m2g useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv x
...
-}
cursorizeProd :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeProd m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex =
  case ex of
  ...
  where
    go intime m1 m2 t x =
      if isPackedContext
        then do
           (x', freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv x
           return (fromDi x', freeVarToVarEnv', m1', m2') 
        else cursorizeExp m1 m2 useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t senv x
...
-}
cursorizeSpawn :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeSpawn m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex = do
  case ex of
    LetE (v, locs, MkTy2 ty, (SpawnE fn applocs args)) bod
      | isPackedTy ty -> do
          (rhs', freeVarToVarEnv', m11, m22) <- do 
                  (expr, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn UnknownTailType applocs args)
      ...
      | hasPacked ty -> do
          (rhs', freeVarToVarEnv', m11, m22) <- do 
                   (expr, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn _cty applocs args)
                   return (fromDi expr, freeVarToVarEnv', m1', m2') 
          let rhs'' = case rhs' of
          ...
          case locs of
            ...
            _ -> do
              ...
              return (mkLets bnds bod', M.union freeVarToVarEnv' freeVarToVarEnv'', m11', m22') 
      | otherwise -> do
          (rhs', freeVarToVarEnv', m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn UnknownTailType applocs args)
          let rhs'' = case rhs' of
                AppE fn' _ applocs' args' -> SpawnE fn' applocs' args'
                _ -> error "cursorizeSpawn"
          case locs of
  ...
  where
    go inTimeIt m1g m2g t s x =
      if isPackedContext
        then do 
            (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g useMutableCursorsCall inTimeIt freeVarToVarEnv lenv ddfs fundefs denv t s x
            return (fromDi x', freeVarToVarEnv', m1g', m2g')
        else cursorizeExp m1 m2 useMutableCursorsCall inTimeIt freeVarToVarEnv lenv ddfs fundefs denv t s x

cursorizeSync :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeSync m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex = do
  case ex of
  ...
  where
    go intime m1g m2g t x =
      if isPackedContext
        then do 
           (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t M.empty x
           return (fromDi x', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1 m2 useMutableCursorsCall intime freeVarToVarEnv lenv ddfs fundefs denv t M.empty x
...
cursorizeLet ::
  ...
  Exp2 ->
  PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeLet m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv (v, locs, (MkTy2 ty), rhs) bod
  | isPackedTy ty = do
      ...
      let ty' = case locs of
          ...
          --                           ) locs
          tys_in_env = [(v, MkTy2 ty), (fresh, MkTy2 ty'), (toEndV v, MkTy2 (projTy 1 ty'))] 
                              ++ map
                                    ( \loc ->
                                      let free_var = fromLocArgToFreeVarsTy loc
                                          var = case (M.lookup free_var freeVarToVarEnv') of
            ...
            [] -> if M.member start_loc m2'
                  ...
                  then
                    ...
                    let mut_loc = dbgTrace (minChatLvl) "Print in cursorizeLet: " dbgTrace (minChatLvl) (sdoc (M.toList m1')) dbgTrace (minChatLvl) "End in cursorizeLet.\n" L.foldr (\(ml, lst) res -> 
                                                            foldr (\ent res' -> case ent of 
                                                                          CurrentScope (vv, vl, erg, _alises) ->        
                                                                                          if vv == start_var
                                                                                          then Just (ml, erg)
                                                                                          else res'
                                                                          FutureScope (vv, vl, erg, _alises) ->        
                                                                                          if vv == start_var
                                                                                          then Just (ml, erg)
                                                                                          else res'
                                                                  ) res lst
            ...
            _ -> if M.member start_loc m2'
                    ...
                    let (loc_bnds, m1'', m2'') = foldr (\(loc, n) (lbndsi, m1i, m2i) -> let loc_var = fromLocArgToFreeVarsTy loc
                                                                                          ...
                                                                                          in case loc of 
                                                                                                                   ...
                                                                                                                   then
                                                                                                                    let output = foldr (\locarg ret -> case locarg of 
                                                                                                                                                              ...
                                                                                                                                                              Loc lrem -> let 
                                                                                                                                                                            ...
                                                                                                                                                                            (mut_loc_lc, m1updated) = findMutableLocationPointingToVar lc_var m1'
                                                                                                                                                                           in case mut_loc_lc of 
                                                                                                                                                                                      Nothing -> ret
                                                                                                                                                                                      Just l -> let mbkey = M.lookup l m1updated
                                                                                                                                                                                                  in case mbkey of 
                                                                                                                                                                                                          Nothing -> ret
                                                                                                                                                                                                          Just lst -> case lst of 
                                                                                                                                                                                                                CurrentScope (_, _, reg, _):xs -> dbgTrace (minChatLvl) "Print in EndOfReg: " dbgTrace (minChatLvl) (sdoc (output, loc)) dbgTrace (minChatLvl) "End printing in EndOfReg FoldrR 1.\n"  reg
                                                                                                                                                                                                                _ -> error "Expected a key in the env!"
                                                                                                                                                              _ -> ret
                                                                                                                                       ) Nothing locs
                                                                                                                     in case output of 
                                                                                                                                                             ...
                                                                                                                                                             EndWitness lrem _ -> let mlrem = lremMode lrem 
                                                                                                                                                                                    in case mlrem of 
                                                                                                                                                                                              InputMutable -> let 
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                (mut_loc_lc, m1updated) = findMutableLocationPointingToVar endName m1'
                                                                                                                                                                                                                loc = toLocVar locarg
                                                                                                                                                                                                               in case mut_loc_lc of 
                                                                                                ...
                                                                                                EndWitness lrem lv -> let witness_loc = lremLoc lrem
                                                                                                                          ...
                                                                                                                          (mut_loc, m1updated) = findMutableLocationPointingToVar witness_var m1
                                                                                                                        in case mut_loc of
                                                                                                                                  -- return no bnds, in case we cannot find a mut_loc
                                                                                                                                  Nothing -> -- check if loc is in the same region.
                                                                                                                                             let mut_loc_in_same_reg = findMutableLocationInSameRegion witness_reg m1updated
                 ...
                 then 
                  ...
                  let mut_loc = L.foldr (\(ml, lst) res -> 
                                          foldr (\ent res' -> case ent of 
                                                                  CurrentScope (vv, vl, erg, _aliases) ->  
                                                                                    if vv == start_var
                                                                                    then Just (ml, erg)
                                                                                    else res'
                                                                  FutureScope (vv, vl, erg, _aliases) ->  
                                                                                    if vv == start_var
                                                                                    then Just (ml, erg)
                                                                                    else res'
                                                ) res lst 
                                      ...
                                      let (loc_bnds, m1'', m2'') = foldr (\(loc, n) (lbndsi, m1i, m2i) -> let loc_var = fromLocArgToFreeVarsTy loc
                                                                                                          ...
                                                                                                          in case loc of 
                                                                                                                                                                                            ...
                                                                                                                                                                                            in case mut_loc_lc of 
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        Just l -> let mbkey = M.lookup l m1updated
                                                                                                                                                                                                                    in case mbkey of 
                                                                                                                                                                                                                            Nothing -> ret
                                                                                                                                                                                                                            Just lst -> case lst of 
                                                                                                                                                                                                                                            CurrentScope (_, _, reg, _):xs -> dbgTrace (minChatLvl) "Print in EndOfReg: " dbgTrace (minChatLvl) (sdoc (output, loc)) dbgTrace (minChatLvl) "End printing in EndOfReg FoldrR 2.\n"  reg
                                                                                                                                                                                                                                            _ -> error "Expected to have values for keys!"
                                                                                                                                                                                _ -> ret
                                                                                                                                                        ) Nothing locs
                                                                                                                                      in case output of
                                                                                                                                              ...
                                                                                                                                              Nothing -> -- find the region from the Output loc
                                                                                                                                                        let (mut_loc_out, m1updated) = findMutableLocationPointingToVar start_var m1'
                                                                                                                                                          in case mut_loc_out of 
                                                                                                                                                                      ...
                                                                                                                                                                      Just l -> let mbkey = M.lookup l m1updated 
                                                                                                                                                                                in case mbkey of 
                                                                                                                                                                                          ...
                                                                                                                                                                                          Just lst -> case lst of  
                                                                                                                                                                                                      CurrentScope (_, _, reg, _):xs -> case reg of 
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      Just rr -> case M.lookup (fromRegVarToFreeVarsTy rr) freeVarToVarEnv' of 
                                                                                                                                                                                                                                                                    Nothing -> case M.lookup (fromRegVarToFreeVarsTy (toEndVRegVar rr)) freeVarToVarEnv' of
                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                    Just rrName -> let m1i' = updateMutableLocPtsToEnv location_var m1i (rrName, Just location_var, reg, S.empty) True
                                                                                                                                                                                ...
                                                                                                                                                                                EndWitness lrem _ -> let mlrem = lremMode lrem 
                                                                                                                                                                                                        in case mlrem of 
                                                                                                                                                                                                                  InputMutable -> let 
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    (mut_loc_lc, m1updated) = findMutableLocationPointingToVar regName m1'
                                                                                                                  ...
                                                                                                                  EndWitness lrem lv -> let witness_loc = lremLoc lrem
                                                                                                                                          ...
                                                                                                                                          in case mut_loc of
                                                                                                                                                            -- return no bnds, in case we cannot find a mut_loc
                                                                                                                                                      Nothing -> let mut_loc_in_same_reg = findMutableLocationInSameRegion witness_reg m1updated 
  ...
  | hasPacked ty = do
      ...
      let cursor_ty_locs' =
            map
              ...
              locs
      (_rhs, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv rhs
  ...
  | otherwise = do
      ...
      let cursor_ty_locs' =
            map
              ...
              locs
      (rhs', freeVarToVarEnv', m1', m2') <- cursorizeExp m1 m2 useMutableCursorsCall insideTimeIt freeVarToVarEnv lenv ddfs fundefs denv tenv senv rhs
      case locs of
        [] -> do 
          (bod', freeVarToVarEnv'', m1'', m2'') <- go insideTimeIt m1' m2' (M.union freeVarToVarEnv' freeVarToVarEnv) (M.insert v (MkTy2 ty) tenv) bod
          return (LetE (v, [], curDict $ stripTyLocs ty, rhs') bod', freeVarToVarEnv'', m1'', m2'')
        _ -> do
          fresh <- gensym "tup_scalar"
          let rhs'' = VarE fresh
              -- Vidush: TODO rename useMutableCursorsCall to something like useMutableCursors... 
              ty' = if useMutableCursorsCall 
                    then cursorizeTy freeVarToVarEnv' m1' m2' useMutableCursorsCall Nothing ty
                    else ProdTy (cursor_ty_locs ++ [cursorizeTy freeVarToVarEnv' m1 m2 useMutableCursorsCall Nothing ty])
                ...
                M.union (M.insert v (MkTy2 ty) tenv) $
                  M.fromList $
                    map
                      ...
                      locs
          (bnds, m1b, m2b) <- if useMutableCursorsCall
                              then do
                                   let (loc_bnds, m1'', m2'') = foldr (\(loc, n) (lbndsi, m1i, m2i) -> let loc_var = fromLocArgToFreeVarsTy loc
                                                                                                           location_var = toLocVar loc
                                                                                                           cursor_ty = cursor_ty_locs' !! n
  ...
  where
    go intime m1g m2g fenv t x =
      if isPackedContext
        then
          do 
            (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g useMutableCursorsCall intime fenv lenv ddfs fundefs denv t senv x
            return (fromDi x', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1g m2g useMutableCursorsCall intime fenv lenv ddfs fundefs denv t senv x
...
unpackDataCon aliveBuffers m1 m2 useMutableCursorsCall insideTimeIt dcon_var freeVarToVarEnv lenv ddfs fundefs denv1 tenv1 senv isPacked scrtCur (dcon, vlocs1, rhs) = do
  ...
  case ty_of_scrut of
    CursorTy -> do
      let (mut_loc, m1updated) = findMutableLocationPointingToVar scrtCur m1
      (m1', bump_bnds) <- case mut_loc of
            ...
            Just l -> do
                      let m1inner = updateMutableLocPtsToEnv l m1updated (field_cur, mut_loc, Nothing, S.empty) False
                      void <- gensym "void"
      ...
      (field_lets, field_v_lst, freeVarToVarEnv', m1', m2', tenv1') <-
        dbgTrace
          ...
          ( \(acc1, acc2, acc3, mi1, mi2, tenvi) (key@(dcon', idx), loc) -> do
              ...
              let curr_window = dbgTrace (minChatLvl) "Print environments: " dbgTrace (minChatLvl) (sdoc (mi2', loc, field_var)) dbgTrace (minChatLvl) "End printing environments after handleIndexingSoACursors.\n" [((dcon', idx), field_var')]
            ...
            ( \(acc1, acc2, acc3, mi1, mi2, tenvi) (key@(dcon', idx), loc) -> do
                ...
                (field_var', field_let, tenvi') <- case scrut_loc_is_mutable of
                                                  True -> do 
                                                           ...
                                                           return (deref_field_var, [(deref_field_var, [], field_cursor_ty', Ext $ AddrOfCursor $ Ext $ IndexCursorArray scrtCur (1 + idx_elem))], M.insert deref_field_var field_cursor_ty2' tenvi)
  ...
  where
                   ...
                   in case ty of
                        ...
                        _ | isScalarTy ty -> do
                          ...
                          if canBind
                            then do
                              ...
                              let (mut_loc, m1updated) = findMutableLocationPointingToVar cur m1
                              m1' <- case mut_loc of 
                                            ...
                                            Just l -> do 
                                                       let m1inner = updateMutableLocPtsToEnv l m1updated (loc_var, Just l, Nothing, S.singleton cur) True
                        ...
                        CursorTy -> do
                          ...
                          (m1', bnds') <- case mut_loc of 
                                            Nothing -> dbgTrace (minChatLvl) "Print inside unpackRegularDataCon AoS Scalar: " dbgTrace (minChatLvl) (sdoc (mut_loc, loc, cur)) dbgTrace (minChatLvl) "End printing inside unpackRegularDcon AoS Scalar!\n" return (m1updated, []) 
                                            Just l -> do 
                                                       let m1inner = updateMutableLocPtsToEnv l m1updated (v, Just l, Nothing, S.empty) False
                          ...
                          if canBind
                            then do
                              ...
                              m1' <- case mut_loc of 
                                            Nothing -> dbgTrace (minChatLvl) "Print inside unpackRegularDataCon AoS Scalar: " dbgTrace (minChatLvl) (sdoc (mut_loc, loc, cur)) dbgTrace (minChatLvl) "End printing inside unpackRegularDcon AoS Scalar!\n" return m1updated 
                                            Just l -> do 
                                                       let m1inner = updateMutableLocPtsToEnv l m1updated (v, Just l, Nothing, S.fromList [cur, loc_var]) True
                                                       dbgTrace (minChatLvl) "Print inside unpackRegularDataCon AoS Scalar: " dbgTrace (minChatLvl) (sdoc (mut_loc, loc, cur)) dbgTrace (minChatLvl) "End printing inside unpackRegularDcon AoS Scalar 2!\n" return m1inner
                          ...
                          if isRedirectionTag dcon
                            then do
                              ...
                              let tenv' =
                                  ...
                                  (mut_loc_pointing_to_dcur, m1updated) = findMutableLocationPointingToVar dcur m1
                                  (binds, m1d) = case mut_loc_pointing_to_dcur of 
                                                      ...
                                                      Just l -> let
                                                                 ...
                                                                 m1' = updateMutableLocPtsToEnv l m1updated (redirection_var_dcon, Just l, Nothing, S.empty) False
                                           ...
                                           in case ty_of_field of
                                                (MkTy2 PackedTy {}) ->
                                                  let (new_binds, m1iout) = case redir_vars of 
                                                                          [v] -> let (var_pts_mutl, m1iupdated) = findMutableLocationPointingToVar var m1i
                                                                                  in case var_pts_mutl of 
                                                                                            Nothing -> ([(redirection_var_flds_variable, [], CursorTy, Ext (AddCursor var (LitE 0)))], m1iupdated)
                                                                                            Just ml -> let m1i' = updateMutableLocPtsToEnv ml m1iupdated (redirection_var_flds_variable, Just ml, Nothing, S.singleton var) True
                                                ...
                                                _ ->
                                                  let (new_binds, m1out) = case redir_vars of 
                                                                        [v] -> let (mut_loc_pts_var, m1iupdated) = dbgTrace (minChatLvl) "Print in redirection SoA: " dbgTrace (minChatLvl) (sdoc (var, m1i)) dbgTrace (minChatLvl) "End in redirection SoA.\n" findMutableLocationPointingToVar var m1i
                                                                                in case mut_loc_pts_var of 
                                                                                          Nothing -> let bnds = [ (vars_next_fields !! index, [], CursorTy, Ext (AddCursor var (LitE 1))),
                                                                                                       ...
                                                                                                       in (bnds, m1iupdated)
                                                                                          Just ml -> let m1i' = updateMutableLocPtsToEnv ml m1iupdated (redirection_var_flds_variable, Just ml, Nothing, S.empty) False
                                   ...
                                   then do 
                                    ...
                                    let tenv' =
                                        ...
                                        (binds, m1d) = case mut_loc_pointing_to_dcur of 
                                                              ...
                                                              Just l -> let 
                                                                          lName = getVarNameFromFreeVar fenv (fromLocVarToFreeVarsTy l)
                                                                          m1' = updateMutableLocPtsToEnv l m1updated (v, Just l, Nothing, S.empty) False
                                                                         in ([ (var_dcon_next, [], CursorTy, Ext (AddCursor dcur (LitE 1))),
                                                                               --(tmp, [], ProdTy [CursorTy, CursorTy, IntTy], read_cursor),
                                                                               -- (v, [], locs_ty3, Ext $ InitCursor locs_ty3),
                                            ...
                                            ( \(index, res, m1i, m2i) ((dcon', idx), var, redir_vars) ->
                                              let read_cursor_f idx =
                                               ...
                                               in case ty_of_field of
                                                    ...
                                                    _ ->
                                                        let (new_binds, m1out) = case redir_vars of 
                                                                                [v] -> let (mut_loc_pts_var, m1iupdated) = findMutableLocationPointingToVar var m1i
                                                                                        in case mut_loc_pts_var of
                                                                                                 Nothing -> let bnds = [ (vars_next_fields !! index, [], CursorTy, Ext (AddCursor var (LitE 1))),
                                                                                                                         (tmpf index, [], ProdTy [CursorTy, CursorTy, IntTy], read_cursor_f index),
                                                                                                                         -- ((loc_var)     , [], CursorTy, VarE dcur),
                                                                                                              ...
                                                                                                              in (bnds, m1iupdated)
                                                                                                 Just ml -> let m1i' = updateMutableLocPtsToEnv ml m1iupdated ((redirection_var_flds !! index), Just ml, Nothing, S.empty) False
                                                                                                                isFieldAlive = S.member (dcon', idx) aliveBuffersi
                  ...
                  let loc = fromLocArgToFreeVarsTy locarg
                   in case ty of
                        ...
                        CursorTy -> do
                          ...
                          let (checkMutLoc, m1gupdated) = findMutableLocationPointingToVar cur m1g 
                          (m1g', add_bnds) <- case checkMutLoc of 
                                          ...
                                          Just ml -> do
                                                     let m1inner = updateMutableLocPtsToEnv ml m1gupdated (toEndV v, Just ml, Nothing, S.empty) False
                                                     void_var <- gensym "void"
                        ...
                        _ | isScalarTy ty -> do
                          locs_var <- lookupVariable loc fenv
                          let (mut_loc, m1gupdated) = findMutableLocationPointingToVar cur m1g
                          m1' <- case mut_loc of
                                            ...
                                            Just l -> do
                                                       let m1inner = updateMutableLocPtsToEnv l m1gupdated (locs_var, Just l, Nothing, S.singleton cur) True
                        ...
                        PackedTy {} -> do
                          ...
                          let (mut_var, m1gupdated) = findMutableLocationPointingToVar cur m1g
    ...
    scalarBinds varAlive fenv m1 m2 ty v loc lcarg tenv = do
      ...
      let (mut_loc, m1gupdated) = findMutableLocationPointingToVar loc m1
      (m1', binds') <- case mut_loc of 
                            ...
                            Just l -> do
                              if varAlive 
                              then do
                                let m1inner = updateMutableLocPtsToEnv l m1gupdated (toEndV v, Just l, Nothing, S.empty) False
                                void_var <- gensym "void"
...
giveStarts tenv fenv useMutableCursorsCall isInsideTimeIt frec mlocptsenv moldenv ty e = do
  case ty of
    ...
    CursorArrayTy{} -> case e of 
                          VarE v -> let (mutloc, mlocptsenvupdated) = findMutableLocationPointingToVar v mlocptsenv
                                     in case mutloc of 
                                              Nothing -> dbgTrace (minChatLvl) "Print in give starts: " dbgTrace (minChatLvl) (sdoc (v, mlocptsenvupdated)) dbgTrace (minChatLvl) "End in give starts Nothing!\n" return e 
                                              Just ml -> do 
                                                         let mlVarName = dbgTrace (minChatLvl) "Print in give starts: " dbgTrace (minChatLvl) (sdoc (v, ml, mlocptsenvupdated)) dbgTrace (minChatLvl) "End in give starts Just ml!\n" getVarNameFromFreeVar fenv (fromLocVarToFreeVarsTy ml)
                                                         return $ VarE mlVarName
                          _ -> return e  
    _ -> case e of 
             VarE vv -> let (mutl, mlocptsenvupdated)  = dbgTrace (minChatLvl) "Print in give starts VarE v case: " dbgTrace (minChatLvl) (sdoc (e)) dbgTrace (minChatLvl) "End in give starts VarE v.\n" findMutableLocationPointingToVar vv mlocptsenv
                         in case mutl of 
                                Nothing -> dbgTrace (minChatLvl) "Print in give starts rest of the case: " dbgTrace (minChatLvl) (sdoc (e, mlocptsenvupdated)) dbgTrace (minChatLvl) "End in give starts rest of the case 1!\n" return e 
                                Just ml -> let mlName = getVarNameFromFreeVar fenv (fromLocVarToFreeVarsTy ml)
                                             in dbgTrace (minChatLvl) "Print in give starts rest of the case: " dbgTrace (minChatLvl) (sdoc (e, mlocptsenvupdated, mlName)) dbgTrace (minChatLvl) "End in give starts rest of the case 2!\n" return $ VarE mlName

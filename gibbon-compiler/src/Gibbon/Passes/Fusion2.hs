   ...
   where
     go ex =
      case ex of
          L1.LetE (v,ls,t, e1) e2 -> L1.LetE (v,ls,t, e1)  (go e2)
          x ->
            let newCall = AppE (fName, TC) [] ( (VarE parName) :outerArgs)
...
inlineConstructorConsumers fdefs exp =
  ...
  where
    go defTable ex  =
       case ex of
          original@(AppE (fName, _) loc parList) ->
...
foldFusedCalls rule@(outerName, innerName, argPos, newName) body =
  let defTable = buildDefTable (body)
      go ex =
        case ex of
          AppE (fName, t) loc argList ->
            let notFolded = AppE (fName, t) loc argList
                  ...
                  if first -- not valid af
                    then
                      do
                        ...
                        let rhs' =  AppE (funName newFun, NoTail) [] args'
...
tupleListOfFunctions  ddefs funcList newName syncedArgs = do
  ...
  return (FunDef newName newArgs (newFuncInputType,newRetType) finalBody (FunMeta NotRec NoInline False NoTail))
 where
   ...
   function{funName=newName, funBody = go (funBody function)}
    where
     go ex =
      ...
      case ex of
          AppE (name, t) loc argList          ->
            AppE (if name==oldName then newName else name, t) loc argList
 ...
 where
  go ex tb = case ex of
      ...
      LetE (boundedVar,_,_,rhs) body    ->
        let tb'=
             case rhs of
                callExp@(AppE (fName, t) _ argList@((VarE inputTree):tail)) ->
            ...
            AppE (v, tc) ls args ->
              if v == funName fdefOld
                then
                  ...
                  let rhs' = AppE (newName, tc) ls args in
                  let
                    body'=  L.foldl
                        (\ex (i, j )->

...
type GibGrowRegionArgs = (Var, Var)
...
data E3Ext loc dec
  ...
  | -- | Create a tagged cursor
    TagCursor Var Var
  | -- | Write a tagged cursor
    WriteTaggedCursor Var (PreExp E3Ext loc dec)
  | -- | Reads and returns a tagged cursor at Var
    ReadTaggedCursor Var
  | -- | Reads and returns the cursor at Var
    ReadCursor Var
  | -- | Write a cursor, and return a cursor
    WriteCursor Var (PreExp E3Ext loc dec)
  | -- | Read a pointer to a linked list
    ReadList Var dec
  | -- | Write a pointer to a linked list
    WriteList Var (PreExp E3Ext loc dec) dec
  | -- | Read a pointer to a vector
    ReadVector Var dec
  | -- | Write a pointer to a vector
    WriteVector Var (PreExp E3Ext loc dec) dec
  | -- | Make a Cursor Array from a list of Cursors. Returns a new variable for Cursor Array.
    MakeCursorArray Int [Var]
  | -- | Index into a Cursor Array
    IndexCursorArray Var Int
  | -- | Add a constant offset to a cursor variable
    AddCursor Var (PreExp E3Ext loc dec)
  | -- | Take the address of a Cursor.
    AddrOfCursor (PreExp E3Ext loc dec)
  | -- | Explicitly de-reference a mutable cursor
    DerefMutCursor Var
  | -- | Cast a pointer to the specified type
    CastPtr Var dec
  | -- | Pointer subtraction
    SubPtr Var Var
  | -- | Create a new buffer, and return a cursor
    NewBuffer L2.Multiplicity
  | -- | Create a temporary scoped buffer, and return a cursor
    ScopedBuffer L2.Multiplicity
  | -- | Create a new buffer for parallel allocations, and return a cursor
    NewParBuffer L2.Multiplicity
  | -- | Create a temporary scoped buffer for parallel allocations, and return a cursor
  ...
  | -- | Bytes required, region, write cursor but for a vector of cursors and regions
    -- We also store the arguments to gibbon grow region in this function.
    BoundsCheckVector [(Int, Var, Var, GibGrowRegionArgs)]
  ...
  | -- | Analogous to L2's extension.
    AllocateTagHere Var TyCon
  | -- | Analogous to L2's extension.
    AllocateScalarsHere Var
  | -- | Marks the beginning of tag allocation.
    StartTagAllocation Var
  | -- | Marks the end of tag allocation.
    EndTagAllocation Var
  | -- | Marks the beginning of scalar allocation.
    StartScalarsAllocation Var
  | -- | Marks the end of scalar allocation.
    EndScalarsAllocation Var
  | SSPush SSModality Var Var TyCon
  | SSPop SSModality Var Var
  | -- | Translates to assert statements in C.
...
instance (HasSimplifiableExt E3Ext l d) => SimplifiableExt (PreExp E3Ext l d) (E3Ext l d) where
  gInlineTrivExt _ _ =
    error $
      "InlineTriv is not a safe operation to perform on L3."
...
mapMExprs fn (Prog ddfs fundefs mainExp) =
  Prog ddfs
    <$> ( mapM
            ( \f@FunDef {funArgs, funTy, funBody} ->

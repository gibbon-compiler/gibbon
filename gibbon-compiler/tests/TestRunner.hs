...
import           OutputCompare ( compareNormalized, compareOutputFiles )
...
(3) If it compiles and produces a answer, we use 'OutputCompare' to check if the
    output matches the answer generated by Racket.
...
A program that uses `iterate`/`timeit` prints nondeterministic timing records
(ITER TIMES / BATCHTIME / SELFTIMED).  Only those complete records are removed
before comparing; every remaining deterministic token, its multiplicity and its
order must still agree.  See Note [Comparing answers] in OutputCompare.

Historically this worked the other way round: any diff that merely mentioned a
timing token was discarded wholesale, so a benchmark could print a wrong value
and still be recorded as a pass (VW-03).  Later, we could add a "bench" mode,
which ensures that the delta is within some reasonable range (like nofib).
...
-- reuses exactly that knob, scoped to a mode, so e.g. a program that behaves
-- differently in one mode can be given a smaller input there instead of
-- recording a mode-specific answer.
...
data Test = Test
    ...
    , compileOnly :: Bool
      ...
      -- benchmark-sized optimization regression tests that need to
...
readMode s =
    case T.toLower s of
        ...
        "int32-vectorize" -> error "readMode: \"int32-vectorize\" has been removed; use the Int32 source type explicitly, and run the gibbon2/loopify/selective/vectorize modes."
        "gibbon-int32-vectorize" -> error "readMode: \"gibbon-int32-vectorize\" has been removed; use the Int32 source type explicitly, and run the gibbon2/loopify/selective/vectorize modes."
    ...
    go mode expected = do
        ...
        case result of
            ...
            Right (outpath, _out) -> do
                -- See Note [Comparing answers] in OutputCompare.  This is the
                -- same normalization the baseline path uses; there is no
                -- second, weaker oracle any more.
                diff_res <- compareOutputFiles outpath anspath
                verdict expected mode diff_res


-- | Mode-specific answer-file infixes, in decreasing order of specificity.
--
-- No active mode currently needs one (this was the removed --int32 modes'
-- hook: a mode whose @Int@ width differed from the baseline could record its
-- own answer file instead of being blanket-listed as @failing:@); kept as a
-- no-op extension point since 'answerPathForTest' below depends on the shape.
  ...
  where
    ...
    baselineAnswerCheck baselinePath =
      case mb_anspath test of
        ...
        Just{} -> do
          ...
          if exists
          then compareOutputFiles baselinePath anspath
    ...
    go baselineOut mode expected = do
        ...
        case result of
          Left err -> verdict expected mode (Just err)
          Right (_outpath, out) ->
              verdict expected mode (compareNormalized "baseline" baselineOut out)
